...
async function insertDataHelper(jsonToken) {
  ...
  playlists_parsed.forEach(async playlist => {
    ...
    let sqlPlaylist ="insert INTO playlist (playlist, tracks) VALUES ('" + playlistName + "','" + tracksInPlaylist +"') ON DUPLICATE KEY UPDATE playlist = '" + playlistName + "', tracks = '" + tracksInPlaylist +"'";
